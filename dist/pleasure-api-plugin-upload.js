...
const PleasureApiPluginUpload = {
  ...
  prepare ({ router, config, mongoose: { Schema } }) {
    ...
    const storagePath = pleasureUtils.findRoot(config.storagePath);
    ...
    pleasureApi.getMongoConnection()
      .then(m => {
        mongoose = m;
    ...
    router.post(postUri, koaUpload, async (ctx, next) => {
      ...
      const FileUpload = {
        ...
        destinationFilename: path.relative(pleasureUtils.findRoot(), destinationFilename),
        remoteIp: ip,
        hash: file.hash
      };
